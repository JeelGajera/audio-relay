...
the rest: builds the Windows executable and the Android APK/AAB, generates
checksums, and publishes a GitHub Release with notes taken from
`CHANGELOG.md`.
...
1. **Update the version in both places.** They are separate files with no
   ...
   - `windows-app/Cargo.toml` → `version`
...
- **No Play Store upload.** The `.aab` is produced so this is a drop-in later
  ...
  publishes it today.
- **Windows only on the desktop side.** Capture is WASAPI, which does not
  exist on macOS or Linux. Supporting them is a new capture backend, not a
  packaging change — see `docs/architecture.md` §2.1.
